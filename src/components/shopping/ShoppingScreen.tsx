...
import { getCart, getProductById, updateQuantity, removeFromCart } from '../../services/shoppingService';
import type { CartItem } from '../../types/shopping';

type ShoppingTab = 'products' | 'sell' | 'profile';
...
export const ShoppingScreen: React.FC<ShoppingScreenProps> = ({ onBack, time }) => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col z-50 shopping-bg">
      ...
      {/* 底部悬浮毛玻璃标签栏 */}
      <div className="absolute bottom-4 left-4 max-w-[70%] bg-[rgba(248,248,250,0.85)] dark:bg-[rgba(30,30,35,0.85)] backdrop-blur-[20px] rounded-2xl shadow-none border border-white/30 dark:border-white/10 py-2 px-4 flex items-center gap-4 z-50">
        <button
        ...
        >
          ...
          <span className="text-[10px] font-bold leading-tight mt-0.5">购物</span>
        </button>
      ...
      <div className="absolute bottom-4 right-4 pointer-events-auto z-50" ref={panelRef}>
        ...
        {isCartPanelOpen && (
          <div 
            className="absolute bottom-[72px] right-0 w-[300px] z-10 transition-all duration-200 origin-bottom-right bg-[rgba(240,240,245,0.85)] dark:bg-[rgba(30,30,35,0.85)] backdrop-blur-[20px] border border-white/80 dark:border-white/10 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.1)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.3)]"
            style={{ animation: 'fadeInZoom 0.2s ease-out' }}
          >
            {/* 尖角指向按钮 */}
            <div className="absolute -bottom-[7px] right-[24px] w-[14px] h-[14px] bg-[rgba(240,240,245,0.85)] dark:bg-[rgba(30,30,35,0.85)] border-r border-b border-white/80 dark:border-white/10 rotate-45 backdrop-blur-[20px] z-0" />
            ...
            <div className="p-4 flex flex-col max-h-[400px] relative z-10">
              ...
              <div className="flex justify-between items-center mb-3 pb-2 border-b border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.1)]">
                <h3 className="text-sm font-bold text-[#1a1a1a] dark:text-[#f0f0f3]">购物车</h3>
                      ...
                      <div key={item.productId} className="flex items-center gap-2 group">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="p-1 text-[#999999] hover:text-red-500 transition-colors bg-transparent opacity-0 group-hover:opacity-100"
                          >
                            <Trash2 size={12} />
                          </button>
                        </div>
        ...
        >
          ...
          {cartItemCount > 0 && (
            <div className="absolute top-0 right-0 min-w-[18px] h-[18px] bg-red-500 rounded-full flex items-center justify-center px-1 border border-white dark:border-[#2a2a2c]">
